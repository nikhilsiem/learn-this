...
insert into storage.buckets (id, name, public) values ('course-uploads', 'course-uploads', true)
  on conflict (id) do nothing;

create policy "Anyone can upload PDFs" on storage.objects
  for insert with check (bucket_id = 'course-uploads'::text);

create policy "Anyone can read PDFs" on storage.objects
  for select using (bucket_id = 'course-uploads'::text);
